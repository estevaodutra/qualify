...
-- badge, not an organizational unit. Only rows that already
-- have a resolved company_id (via the previous backfill
-- migration) are indexed -- a definition with no company_id
-- would be unreachable under workflow_definitions' RLS anyway.
-- =====================================================
...
WHERE ds.company_id IS NOT NULL
...
WHERE ms.company_id IS NOT NULL
ON CONFLICT (source_type, source_id) DO NOTHING;
...
WHERE call.company_id IS NOT NULL
ON CONFLICT (source_type, source_id) DO NOTHING;
