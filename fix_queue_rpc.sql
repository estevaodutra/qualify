...
BEGIN
  ...
  WITH combined AS (
    ...
    FROM call_queue cq
    LEFT JOIN call_campaigns cc ON cc.id = cq.campaign_id
    WHERE cq.company_id = p_company_id
      AND cq.status = 'waiting'
      AND (p_campaign_ids IS NULL OR cq.campaign_id = ANY(p_campaign_ids) OR (array_length(p_campaign_ids, 1) = 0))
    ...
    WHERE cl.company_id = p_company_id
      ...
      AND (p_campaign_ids IS NULL OR cl.campaign_id = ANY(p_campaign_ids) OR (array_length(p_campaign_ids, 1) = 0))
      AND (p_attempt_filter IS NULL
