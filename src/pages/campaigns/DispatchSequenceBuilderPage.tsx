...
export default function DispatchSequenceBuilderPage() {
  const { campaignId, sequenceId } = useParams<{ campaignId: string; sequenceId: string }>();
  const navigate = useNavigate();
  const { sequences, isLoading, updateSequence } = useDispatchSequences(campaignId);

  const sequence = sequences.find(s => s.id === sequenceId);
  const handleBack = () => navigate("/campaigns/whatsapp/despacho");
  ...
  return (
    <div className="fixed inset-0 h-[100dvh] w-screen overflow-hidden flex flex-col bg-[#F8F9FC] p-3">
      {isLoading ? (
        ...
        <DispatchSequenceBuilder sequence={sequence} onBack={handleBack} onUpdate={updateSequence} />
      ) : (
